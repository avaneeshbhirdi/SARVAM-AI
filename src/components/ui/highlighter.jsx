  ...
  const onMouseMove = () => {
    if (containerRef.current) {
      ...
      if (inside) {
        mouse.current.x = x;
        mouse.current.y = y;
        Array.from(containerRef.current.children).forEach((box) => {
...
export const HighlighterItem = ({ children, className = "" }) => {
  return (
    <div
      className={`relative overflow-hidden p-px before:pointer-events-none before:absolute before:-left-48 before:-top-48 before:z-30 before:h-96 before:w-96 before:translate-x-[var(--mouse-x)] before:translate-y-[var(--mouse-y)] before:rounded-full before:bg-amber-400 before:opacity-0 before:blur-[100px] before:transition-opacity before:duration-500 after:absolute after:inset-0 after:z-10 after:rounded-3xl after:opacity-0 after:transition-opacity after:duration-500 group-hover:before:opacity-30 group-hover/item:after:opacity-100 group/item ${className}`}
    >
      {children}
    </div>
  );
